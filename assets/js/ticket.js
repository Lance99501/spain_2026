...
export function createTicketController({encryptedTickets,sessionMinutes=5}){
  ...
  let activeTicketId='',activeTicketLabel='';
  ...
  function saveSession(password){
    sessionStorage.setItem(SESSION_KEY,JSON.stringify({password,expiresAt:Date.now()+SESSION_MS}));
  }
  ...
  async function showResult(password){
    const payload=encryptedTickets[activeTicketId];
    if(!payload) throw new Error('ticket-not-found');
    ...
    if(ticketResultTitle) ticketResultTitle.textContent=activeTicketLabel;
    if(ticketOpenLink) ticketOpenLink.href=url;
    if(ticketQr) ticketQr.innerHTML='';

    if(window.QRCode && ticketQr){
      new QRCode(ticketQr,{text:url,width:220,height:220,correctLevel:QRCode.CorrectLevel.M});
    }else if(ticketQr){
  ...
  async function open(id,label){
    ...
    if(session){
      if(ticketModalSub) ticketModalSub.textContent=label+`｜已解鎖，${sessionMinutes} 分鐘內免再輸入`;
      if(ticketUnlockForm) ticketUnlockForm.style.display='none';
    ...
    if(ticketModalSub) ticketModalSub.textContent=label+'｜輸入老婆生日後顯示 QR Code';
  ...
  ticketModal?.addEventListener('click',e=>{if(e.target===ticketModal) close();});
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&ticketModal?.classList.contains('open')) close();});

  ticketUnlockForm?.addEventListener('submit',async e=>{
    e.preventDefault();
    ...
    try{
      ...
      if(ticketModalSub) ticketModalSub.textContent=activeTicketLabel+`｜已解鎖，${sessionMinutes} 分鐘內免再輸入`;
    }catch{
      clearSession();
